...
from OpenGL.GLUT import GLUT_BITMAP_9_BY_15, GLUT_BITMAP_HELVETICA_18, GLUT_BITMAP_HELVETICA_12
...
class Camera:
    ...
    def move(self, key, world):
        ...
        if key == b'd':
            new_pos[0] -= math.sin(rad) * self.speed
            new_pos[2] += math.cos(rad) * self.speed
        if key == b'q': new_pos[1] += self.speed
        if key == b'e': new_pos[1] -= self.speed
